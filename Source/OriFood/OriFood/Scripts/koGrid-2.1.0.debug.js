...
(function (window, undefined) {
    ...
    ***********************************************/
    ...
    var TEMPLATE_REGEXP = /<.+>/;
    var KG_ROW_SELECTED = '_kg_row_selected_';
    var KG_ROW_VISIBLE = '_kg_row_visible_';
    var KG_ROW_EXPANDED = '_kg_row_expanded_';
    var KG_ROW_CLASSES = '_kg_row_classes_'
    var SUB_ITEMS_PROP = '_kg_has_children_';
    var ROW_DEPTH_PROP = '_kg_row_depth_';

    var LABEL_TEXT = {
        SelectedItems: 'Số mục đã chọn',
        TotalItems: 'Tổng số mục',
        PageSize: 'Số mục hiển thị trên trang',
        ChooseColumns: 'Chọn cột hiển thị',
        FilterTextPlaceHolder: 'Giá trị tìm kiếm nhanh',
        GroupBy: 'Nhóm cột',
        DragToGroupBy: 'Kéo cột vào đây để nhóm theo cột'
    }
    ...
    ***********************************************/
    kg.defaultGridTemplate = function () {
        ...
        '<div class="kgTopFunction" data-bind="style: $data.topFunctionStyle, visible: enableExcelExporting">' +
            '<button type="button" class="btn btn-default" data-bind="visible: enableExcelExporting, click: function(){ exportToExcel(false); }">Xuất trang hiện tại</button>' +
            '<button type="button" class="btn btn-default" data-bind="visible: enableExcelExporting, click: function(){ exportToExcel(true); }">Xuất tất cả</button>' +
        '</div>' +
        '<div class="kgTopPanel" data-bind="css: {\'ui-widget-header\':jqueryUITheme, \'ui-corner-top\': jqueryUITheme}, style: $data.topPanelStyle">' +
            '<div class="kgGroupPanel" data-bind="visible: $data.showGroupPanel, style: headerStyle">' +
                '<div class="kgGroupPanelDescription" data-bind="visible: configGroups().length == 0">' +
                    '' + LABEL_TEXT.DragToGroupBy + '</div>' +
                    ...
                    '<li class="kgGroupItem">' +
                        '<span class="kgGroupElement">' +
                            '<div class="kgGroupName">' +
                                '<span data-bind="text: displayName" style="float: left">' +
                                '</span>' +
                                '<span data-bind="click: function(data) { $root.removeGroup($index()) }" class="kgRemoveGroup">' +
                                'x</span>' +
                            '</div>' +
                            '<span data-bind="visible: $index() < ($root.configGroups().length - 1)" class="kgGroupArrow">' +
                            '</span>' +
                        '</span>' +
                    '</li>' +
                '</ul>' +
        ...
        '<div class="kgHeaderContainer" data-bind="style: headerStyle">' +
            '<div class="kgHeaderScroller" data-bind="style: headerScrollerStyle, kgHeaderRow: $data" >' +
            '</div>' +
        '</div>' +
        '<div class="kgHeaderButton" data-bind="visible: false, click: toggleShowMenu">' +
            '<div class="kgHeaderButtonArrow">' +
            '</div>' +
        '</div>' +
        '<div data-bind="visible: showMenu" class="kgColMenu">' +
            //'<div data-bind="visible: showFilter">' +
            //    '<input placeholder="' + LABEL_TEXT.FilterTextPlaceHolder + '" type="text" data-bind="value: filterText, valueUpdate: \'afterkeydown\'"/>' +
            //'</div>' +
            '<div data-bind="visible: showColumnMenu">' +
                '<span class="kgMenuText">' +
                '' + LABEL_TEXT.ChooseColumns + ':</span>' +
                '<ul class="kgColList" data-bind="foreach: nonAggColumns">' +
                    '<li class="kgColListItem">' +
                        '<label style="position: relative; width: 100%">' +
                            ...
                            '<span data-bind="text: displayName, click: toggleVisible">' +
                            '</span>' +
                            //'<a title="' + LABEL_TEXT.GroupBy + '" data-bind="attr: {\'class\': groupedByClass }, visible: (field != \'✔\'), click: $parent.groupBy">' +
                            //'</a>' +
                            //'<span class="kgGroupingNumber" data-bind="visible: groupIndex() > 0, text: groupIndex">' +
                            //'</span>' +
                        '</label>' +
                    '</li>' +
                '</ul>' +
    ...
    '<div class="kgViewport" data-bind="css: {\'ui-widget-content\': jqueryUITheme}, style: viewportStyle">' +
        '<div class="kgCanvas" data-bind="style: canvasStyle">' +
            '<div data-bind="foreach: renderedRows" style="position: absolute;">' +
                '<div data-bind="visible: visible, style: { \'position\': \'relative\', \'height\': $parent.rowHeight + \'px\' }, click: toggleSelected, css: {\'selected\': highlight, \'even\': isEven , \'odd\': isOdd, \'ui-state-default\': $parent.jqueryUITheme && isOdd, \'ui-state-active\':$parent.jqueryUITheme && isEven}, kgRow: $data" class="kgRow">' +
                '</div>' +
            '</div>' +
        '</div>' +
    '</div>' +
    '<div class="kgFooterPanel" data-bind="css: {\'ui-widget-content\': jqueryUITheme, \'ui-corner-bottom\': jqueryUITheme}, style: footerStyle">' +
        '<div class="kgTotalSelectContainer" data-bind="visible: footerVisible">' +
            '<div class="kgFooterTotalItems" style="margin-top: 3px" data-bind="css: {\'kgNoMultiSelect\': !multiSelect}" >' +
                '<span class="kgLabel">' +
                    '' + LABEL_TEXT.TotalItems + ': <span data-bind="text: maxRowsDisplay">' +
                    '</span>' +
                '</span>' +
                '<span data-bind="visible: filterText().length > 0" class="kgLabel">' +
                    '(Showing: <span data-bind="text: totalFilteredItemsLength">' +
                    '</span>' +
                ')</span>' +
            '</div>' +
            '<div class="kgFooterSelectedItems" style="margin-top: 3px" data-bind="visible: multiSelect">' +
                '<span class="kgLabel">' +
                    '' + LABEL_TEXT.SelectedItems + ': <span data-bind="text: selectedItemCount">' +
                    '</span>' +
                '</span>' +
            '</div>' +
        '</div>' +
        '<div class="kgPagerContainer" style="float: right; margin-top: 10px;" data-bind="visible: (footerVisible && enablePaging), css: {\'kgNoMultiSelect\': !multiSelect}">' +
            '<div style="float:left; margin-right: 10px;" class="kgRowCountPicker">' +
                '<span style="float: left; margin-top: 3px;" class="kgLabel">' +
                '' + LABEL_TEXT.PageSize + ':</span>' +
                '<select style="float: left;height: 25px; width: 60px" data-bind="value: pagingOptions.pageSize, options: pagingOptions.pageSizes">' +
                '</select>' +
            '</div>' +
            '<div style="float:left; margin-right: 10px; line-height:25px;" class="kgPagerControl" style="float: left; min-width: 135px;">' +
                '<button class="kgPagerButton" data-bind="click: pageToFirst, disable: cantPageBackward()" title="First Page">' +
                    '<div class="kgPagerFirstTriangle">' +
                        '<div class="kgPagerFirstBar">' +
                        '</div>' +
                    '</div>' +
                '</button>' +
                '<button class="kgPagerButton" data-bind="click: pageBackward, disable: cantPageBackward()" title="Previous Page">' +
                    '<div class="kgPagerFirstTriangle kgPagerPrevTriangle">' +
                    '</div>' +
                '</button>' +
                '<select class="kgPagerCurrent" style="width:50px; height: 25px; padding: 0px 4px; vertical-align: top" data-bind="value: pagingOptions.currentPage, valueUpdate: \'change\', options: pageOptions"></select>' +
        //'<input class="kgPagerCurrent" type="number" style="width:50px; height: 24px; margin-top: 1px; padding: 0px 4px;" data-bind="value: pagingOptions.currentPage, valueUpdate: \'change\', attr: { min: 1, max: Math.max(maxPages(), 1) }"/>' +
        '<button class="kgPagerButton" data-bind="click: pageForward, disable: cantPageForward()" title="Next Page">' +
            '<div class="kgPagerLastTriangle kgPagerNextTriangle">' +
            '</div>' +
        '</button>' +
            '<button class="kgPagerButton" data-bind="click: pageToLast, disable: cantPageForward()" title="Last Page">' +
            '<div class="kgPagerLastTriangle">' +
                '<div class="kgPagerLastBar">' +
                '</div>' +
            '</div>' +
        '</button>' +
    '</div>' +
...
'</div>';
    ...
    ***********************************************/
    kg.defaultRowTemplate = function () {
        return '<div data-bind="attr: { \'class\': rowClasses() }"><div data-bind="css: { \'ui-widget-content\': $grid.jqueryUITheme }"></div><!-- ko foreach: $grid.visibleColumns -->' +
            '<!-- ko if: $index() == 0 --><div data-bind="visible: $parent.hasChildren, click: $parent.toggleChildren"><div data-bind="css: $parent.toggleChildrenClass, style: { left: $parent.offsetLeft() + \'px\' }"></div></div><div data-bind="attr: { \'class\': cellClass() + \' kgCell first col\' + $index() }, kgCell: $data, style: { left: (parseInt($parent.offsetLeft()) + ($parent.$grid.config.isExpandable ? 10 : 0)) + \'px\' , width: function(){ return (_width() + parseInt($parent.offsetWidth())) + \'px\'; }() }"></div><!-- /ko -->' +
            '<!-- ko if: $index() != 0 --><div data-bind="attr: { \'class\': cellClass() + \' kgCell col\' + $index() }, kgCell: $data"></div><!-- /ko -->' +
            '<!-- /ko --></div><div></div></div>';
    };
    ...
    ***********************************************/
    kg.defaultCellTemplate = function () { return '<div data-bind="attr: { title: $data.getProperty($parent), \'class\': \'kgCellText colt\' + $index()}, html: $data.getProperty($parent)"></div>'; };
    ...
    ***********************************************/
    kg.Column = function (config, grid) {
        ...
        self._width = ko.computed(function () { return self.width; });
        ...
        self.dataType = ko.observable(colDef.dataType);
        ...
        self.exportable = ko.observable(kg.utils.isNullOrUndefined(colDef.exportable) || colDef.exportable);
        ...
        self.toggleVisible = function (val) {
            ...
            if (typeof (grid.config.toggleVisible) == "function") {
                grid.config.toggleVisible(colDef, v);
            }
        };
    ...
    kg.EventProvider = function (grid) {
        ...
        self.setDraggables = function () {
            if (!grid.config.jqueryUIDraggable) {
                ...
                [].forEach.call(grid.$root.find('.kgHeaderSortColumn'), function (col) {
    ...
    ***********************************************/
    kg.RowFactory = function (grid) {
        ...
        self.rowConfig = {
            ...
            loadChildren: grid.config.loadChildren,
            isExpandable: grid.config.isExpandable,
            toggleVisible: grid.config.toggleVisible
        };
        ...
        self.buildEntityRow = function (entity, rowIndex, rowOffset) {
            rowOffset = rowOffset || 0;
            var row = self.rowCache[rowIndex]; // first check to see if we've already built it
            if (!row) {
                // build the row

                row = new kg.Row(grid, entity, self.rowConfig, self.selectionService);
                row.rowIndex(rowIndex + 1); //not a zero-based rowIndex
                row.offsetTop((self.rowHeight * rowIndex).toString() + 'px');
                row.hasChildren(entity[SUB_ITEMS_PROP] || false);
            ...
            //else {
            //    row.rowIndex(rowIndex + 1); //not a zero-based rowIndex
            //    row.offsetTop((self.rowHeight * (rowIndex - rowOffset)).toString() + 'px');
            //}
            return row;
        };
        ...
        self.renderedChange = function () {
            ...
            $.each(dataArray, function (indx, item) {
                ...
                //}
                if (item.isAggRow) {
                    row = self.buildAggregateRow(item, aggIndex);
                    aggIndex++;
                } else {
                    row = self.buildEntityRow(item, rowIndex);
                    rowIndex++;
                }
        ...
        self.renderedChangeNoGroups = function () {
            ...
            $.each(dataArr, function (i, item) {
                //var row = self.buildEntityRow(item, self.renderedRange.topRow + i, rowOffset);
                var row = self.buildEntityRow(item, i, rowOffset);
                for (var i = rowArr.length - 1; i >= 0; i--) {
                    if (rowArr[i].rowDepth == row.rowDepth - 1) {
                        row.parent(rowArr[i]);
                        break;
                    }
                }
                //if (item[KG_HIDDEN] == true) {
                //    rowOffset++;
                //}
    ...
    ***********************************************/
    kg.Grid = function (options) {
        var defaults = {
            ...
            multiSelect: options.multiSelect == undefined ? true : options.multiSelect,
            tabIndex: -1,
            ...
            groups: [],
            showGroupPanel: options.showGroupPanel == undefined ? false : options.showGroupPanel,
            enableRowReordering: false,
            showColumnMenu: true,
            showFilter: options.multiSelect == undefined ? false : options.multiSelect,
            ...
            toggleVisible: options.toggleVisible
        },
            self = this;
        ...
        self.calcMaxCanvasHeight = function () {
            return (self.configGroups().length > 0) ? (self.rowFactory.parsedData.filter(function (e) {
                return e[KG_HIDDEN] === false;
            }).length * self.config.rowHeight) : (self.filteredData().filter(function (e) {
                return e[KG_HIDDEN] === false;
        ...
        self.buildColumnDefsFromData = function () {
            ...
            kg.utils.forIn(item, function (prop, propName) {
                if (propName != SELECTED_PROP && propName != SUB_ITEMS_PROP) {
        ...
        self.buildColumns = function () {
            ...
            if (self.config.displaySelectionCheckbox && columnDefs.length > 0 && columnDefs[0].field != '\u2714') {
        ...
        self.configureColumnWidths = function () {
            ...
            $.each(self.configGroups(), function (index, item) {
                cols.splice(0, 0, {
                    field: '\u2714',
                    width: self.elementDims.rowSelectedCellW,
                    sortable: false,
        ...
        self.renderedVisibleRows = ko.computed(function () {
            return ko.utils.arrayFilter(self.renderedRows(), function (row) {
                return row.visible();
            });
        });
        self.headerRow = null;
        ...
        // Functions
        self.enableExcelExporting = ko.observable(self.config.enableExcelExporting);
        self.exportToExcel = self.config.exportToExcel || function () { },
        ...
        self.topFunctionPanelHeight = ko.observable(self.config.enableExcelExporting == true ? (self.config.headerRowHeight) : 0);
        self.viewportDimHeight = ko.computed(function () {
            return Math.max(0, self.rootDim.outerHeight() - self.topPanelHeight() - self.topFunctionPanelHeight() - self.config.footerRowHeight + 10);
        });
        ...
        self.pageOptions = ko.computed(function () {
            var options = []
            for (var i = 1; i <= Math.max(self.maxPages(), 1) ; i++) {
                options.push(i);
            }
            return options
        });
        ...
        //append new data (sub-items) to specific item
        self.pushArray = function (data, item) {
            var index = isNaN(item) ? self.sortedData.indexOf(item) : item;
            if (data instanceof Array && index > -1) {
                index++;
                var rowDepth = item[ROW_DEPTH_PROP] || 0;
                rowDepth++;
                for (var i = 0; i < data.length; i++) {
                    data[i][ROW_DEPTH_PROP] = rowDepth;
                    self.sortedData.splice(index, 0, data[i]);
                }
            }
        }

        self.push = function (data, item) {
            var index = isNaN(item) ? self.sortedData.indexOf(item) : item;
            if (index > -1) {
                index++;
                ...
                var rowDepth = item[ROW_DEPTH_PROP] || 0;
                rowDepth++;
                data[ROW_DEPTH_PROP] = rowDepth;
                self.sortedData.splice(index, 0, data);
            }
        }

        self.maximumRowDepth = ko.computed(function () {
            var rowDepth = 0;

            ko.utils.arrayForEach(self.renderedRows(), function (row) {
                var depth = parseInt(row.entity[ROW_DEPTH_PROP]);
                if (!isNaN(depth) && depth > rowDepth) {
                    rowDepth = depth;
                }
            });
    ...
    ***********************************************/
    kg.Row = function (grid, entity, config, selectionService) {
        var self = this, // constant for the selection property that we add to each data item
            canSelectRows = config.canSelectRows;

        //self.rowClasses = ko.observable(config.rowClasses);
        self.rowClasses = ko.observable(entity[KG_ROW_CLASSES]);
        self.selectedItems = config.selectedItems;
        self.entity = entity;
        self.selectionService = selectionService;

        self.selected = ko.observable(entity[KG_ROW_SELECTED] || false);
        self.selected.subscribe(function (value) {
            entity[KG_ROW_SELECTED] = value;
            //if (value) {
            //    self.highlight(value);
            //}
        });
        self.highlight = ko.observable(self.selected());
        ...
        if (self.selected()) {
            self.selectionService.selectedItems.push(entity);
        }
        self.toggleSelected = function (row, event) {
            ...
            if (element.type == "checkbox") {
                ...
                if ($(element).hasClass("kgAggArrowExpanded") || $(element).hasClass("kgAggArrowCollapsed")) {
                    return true;
                } else {
        ...
        self.rowDepth = entity[ROW_DEPTH_PROP] || 0;
        self.offsetLeft = ko.computed(function () {
            return (self.rowDepth * 10).toString();
        });
        self.offsetWidth = ko.computed(function () {
            return (grid.maximumRowDepth() - self.rowDepth) * 10;
        });
        self.parent = ko.observable();
        self.hasChildren = ko.observable(false);
        self.visible = ko.computed(function () {
            return self.parent() == undefined || (self.parent().toggleChildrenExpanded() && self.parent().visible());
        });
        self.visible.subscribe(function (newVal) {
            self.entity[KG_HIDDEN] = !newVal;
        });
        self.toggleChildrenExpanded = ko.observable(entity[KG_ROW_EXPANDED] || false);
        self.childrenLoaded = ko.observable(self.toggleChildrenExpanded());
        self.toggleChildrenExpanded.subscribe(function (expanded) {
            if (expanded) {
                // Load children if haven't

                if (typeof (config.loadChildren) == "function" && self.hasChildren() && !self.childrenLoaded()) {
                    config.loadChildren(self.entity);
                    self.childrenLoaded(true);
                }
            }
            entity[KG_ROW_EXPANDED] = expanded;
            $(window).trigger('resize');
        });
        self.toggleChildrenClass = ko.computed(function () {
            return self.toggleChildrenExpanded() ? 'kgAggArrowExpanded' : 'kgAggArrowCollapsed';
        });
        self.toggleChildren = function () {
            self.toggleChildrenExpanded(!self.toggleChildrenExpanded());
        };
    ...
    ***********************************************/
    kg.StyleProvider = function (grid) {
        ...
        grid.topFunctionStyle = ko.computed(function () {
            return { "height": grid.config.headerRowHeight + "px" };
        });
    ...
    ***********************************************/
    ...
    kg.domUtilityService = {
        AssignGridContainers: function (rootEl, grid) {
            grid.$root = $(rootEl);
            //Headers
            grid.$topFunctionPanel = grid.$root.find(".kgTopFunction");
        ...
        BuildStyles: function (grid) {
            ...
            var rowHeight = grid.config.rowHeight,
                ...
                sumWidth = (grid.config.isExpandable ? 10 : 0) + (grid.maximumRowDepth() * 10);
