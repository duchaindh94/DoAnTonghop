...
ko.bindingHandlers['html'] = {
    ...
    'update': function (element, valueAccessor, allBindingsAccessor) {
        if (allBindingsAccessor().wrapText) {
            
            var value = (ko.utils.unwrapObservable(valueAccessor()) || '').replace(new RegExp('\r?\n', 'g'), '<\/br>');
...
ko.bindingHandlers['text'] = {
    'init': function (element) {        
        ...
        if (element.tagName && element.tagName.toLowerCase() == 'textarea' && $.fn.autosize) {
            if (!$(element).attr("maxLength")) {
                $(element).attr("maxLength", "4000");
            }
            $(element).autosize();
        }
        if (element.tagName && element.tagName.toLowerCase() == 'input' && element.type.toLowerCase() == "text") {
            if (!$(element).attr("maxLength")) {
                $(element).attr("maxLength", "200");
            }
        }
        return { 'controlsDescendantBindings': true };
    },
